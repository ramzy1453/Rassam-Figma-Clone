  ...
  if (clipboardData) {
    try {
      ...
      parsedObjects.forEach((objData: FabricObject) => {
        ...
        util.enlivenObjects([objData]).then(([fabricObject]) => {
          const enlivenedObj = fabricObject as FabricObject;
          enlivenedObj.set({
            left: enlivenedObj.left + 20 || 0 + 20,
            top: enlivenedObj.top + 20 || 0 + 20,
            objectId: uuidv4(),
            fill: "#aabbcc",
          });
